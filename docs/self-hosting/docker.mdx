...
<Warning>
  **Repository migration planned.** The GitHub repository is moving to `reactive-resume/app`.
  Docker Hub stays at `amruthpillai/reactive-resume`; GHCR is moving to `ghcr.io/reactive-resume/app`.
  The GHCR examples below use the new address. Keep your current image configuration until the
  [pinned migration announcement](https://github.com/amruthpillai/reactive-resume/issues/3503) confirms availability.
</Warning>
...
After the new GHCR image is confirmed available, replace `ghcr.io/amruthpillai/reactive-resume` with
